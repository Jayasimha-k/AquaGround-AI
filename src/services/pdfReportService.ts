...
export function generateCGWBReportPDF(data: ReportData) {
  ...
  const recharge = data.recharge_mcm ?? 120.5;
  const extraction = data.extraction_mcm ?? 165.2;
  const netBalance = (recharge - extraction).toFixed(1);
  const isDeficit = (recharge - extraction) < 0;
  ...
  const html = `
    ...
    <html>
    <head>
      <title>CGWB Annual Hydrological Assessment & Extraction-Recharge Report - ${data.district}</title>
      <style>
        body {
    ...
    <body>
      <div class="header">
        ...
        <div class="title">Annual Hydrological Assessment & Resource Audit</div>
        <div class="subtitle">Generated by AquaGround AI DSS • ${new Date().toLocaleDateString('en-IN', { dateStyle: 'full' })}</div>
      </div>

      <div class="grid">
        <div class="card">
          <div class="card-title">Target Jurisdiction</div>
          <div class="card-value" style="font-size:15px;">${data.district}, ${data.state}</div>
        </div>
        <div class="card">
          <div class="card-title">Moratorium Status</div>
        ...
        <div class="card">
          ...
          <div class="card-value" style="color:#10b981;">${recharge} MCM</div>
        </div>
        <div class="card">
          <div class="card-title">Annual Extraction</div>
          <div class="card-value" style="color:#ef4444;">${extraction} MCM</div>
        </div>
      </div>

      <!-- ANNUAL EXTRACTION VS RECHARGE TABLE -->
      <div class="section">
        <div class="section-title">Annual Ground Water Resources & Extraction Assessment</div>
        <div class="table-container">
          <table>
            <thead>
              <tr>
                <th>Hydrological Assessment Metric</th>
                <th>Annual Quantum (MCM/year)</th>
                <th>Status & Category</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td><strong>Annual Natural Aquifer Recharge</strong></td>
                <td style="color:#10b981; font-weight:700;">${recharge} MCM</td>
                <td>Monsoon & Non-Monsoon Replenishment</td>
              </tr>
              <tr>
                <td><strong>Annual Total Groundwater Extraction</strong></td>
                <td style="color:#ef4444; font-weight:700;">${extraction} MCM</td>
                <td>Irrigation, Industrial & Domestic Withdrawals</td>
              </tr>
              <tr>
                <td><strong>Net Annual Ground Water Balance</strong></td>
                <td style="color:${isDeficit ? '#ef4444' : '#10b981'}; font-weight:800;">${netBalance} MCM</td>
                <td>${isDeficit ? 'Over-Exploited Storage Deficit' : 'Sustainable Storage Surplus'}</td>
              </tr>
              <tr>
                <td><strong>Stage of Ground Water Extraction (SOE)</strong></td>
                <td style="font-weight:700;">${data.soe_pct}%</td>
                <td>${data.soe_pct > 100 ? 'Critical / Over-Exploited (>100%)' : 'Semi-Critical / Safe (<100%)'}</td>
              </tr>
              <tr>
                <td><strong>DWLR Water Table Depth</strong></td>
                <td>${data.water_level_mbgl} m bgl</td>
                <td>Below Ground Level Sensors Average</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
          <div>
            <div style="display: flex; justify-content: space-between; font-size: 11px; font-weight: 700; color: #334155;">
              <span>Groundwater Sustainability Index (GSI)</span>
